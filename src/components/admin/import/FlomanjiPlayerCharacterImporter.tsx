...
interface FlomanjiPlayerCharacterImporterProps {
  onComplete?: () => void;
}

export function FlomanjiPlayerCharacterImporter({ onComplete }: FlomanjiPlayerCharacterImporterProps) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      console.error("Error importing player character cards:", error);
      setImportResult({
        success: false,
        count: 0,
  ...
  return (
    <>
      ...
      >
        ...
        <span>Import Player Characters</span>
          ...
          <DialogHeader>
            <DialogTitle>
              {importResult.success
                ? "Player Characters Imported Successfully"
                : "Import Completed with Issues"}
            </DialogTitle>
            <DialogDescription>
              {importResult.success
                ? `Successfully imported ${importResult.count} Flomanji player character cards into the database.`
                : "There were some issues importing the player character cards."}
            </DialogDescription>
          </DialogHeader>
